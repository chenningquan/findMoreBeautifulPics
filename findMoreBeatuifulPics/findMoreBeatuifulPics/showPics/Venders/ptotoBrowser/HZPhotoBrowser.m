...
- (void)saveImage
{
    ...
    if (currentView.hasLoadedImage) {
        ...
        if ([currentView.imageview isKindOfClass:[UIImageView class]]) {
            animatedImageView = (UIImageView *)currentView.imageview;
            NSDate *imageData = UIImagePNGRepresentation(animatedImageView.image);
            if (!imageData) {
